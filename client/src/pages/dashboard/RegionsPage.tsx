...
import { createRegion, deleteRegion, getRegions, restoreRegion, forceDeleteRegion, updateRegion, type Region } from "../../services/regions";
...
export default function RegionsPage() {
  ...
  const [form, setForm] = useState<{ name_ar: string; name_fr: string }>({ name_ar: "", name_fr: "" });
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const data = await getRegions(status);
        setRows(data);
      } catch {
        setError("تعذر تحميل الجهات");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const filtered = useMemo(() => {
    ...
    if (!t) return rows;
    return rows.filter(r => r.name_ar.includes(q) || r.name_fr.toLowerCase().includes(t));
  }, [rows, q]);
  ...
  const openEdit = (r: Region) => {
    setEditing(r);
    setForm({ name_ar: r.name_ar, name_fr: r.name_fr });
    setModalOpen(true);
  };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (editing) {
        const updated = await updateRegion(editing.id, form);
  ...
  const onForceDelete = (id: number) => {
    setConfirm({ id, type: 'force' });
  };
  ...
  const confirmAction = async () => {
    ...
    try {
      if (confirm.type === 'archive') {
        await deleteRegion(confirm.id);
      } else {
        await forceDeleteRegion(confirm.id);
      }
  ...
  return (
    <DashboardLayout title="الجهات المغربية">
      <section className="rounded-2xl border border-orange-200/60 bg-white p-0 shadow-sm" dir="rtl">
        <div className="flex items-center justify-between gap-2 rounded-t-2xl border-b border-orange-200/50 bg-gradient-to-br from-orange-50 to-amber-50 px-4 py-3">
          <div>
            <h3 className="text-sm font-extrabold tracking-tight text-orange-900">الجهات</h3>
            <p className="text-xs text-gray-600">إدارة الجهات المغربية</p>
          </div>
          ...
          <div className="overflow-x-auto px-4 pb-4">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y">
                {filtered.map((r) => (
                  <tr key={r.id} className="transition hover:bg-orange-50/40">
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-3">
                        <div className="inline-flex size-9 items-center justify-center rounded-lg bg-gradient-to-br from-orange-200 to-amber-200 text-sm font-bold text-orange-900 ring-1 ring-orange-300/60">
                          {String(r.name_ar.charAt(0))}
                        </div>
                        <div className="leading-tight">
                          <div className="font-semibold text-gray-900">{r.name_ar}</div>
                          <div className="text-[11px] text-gray-500">ID: {r.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-3 text-gray-700">{r.name_fr}</td>
                    ...
                    <td className="px-3 py-3 text-end">
                      {status !== 'archived' ? (
                        <div className="inline-flex items-center gap-1">
                          <button title="تعديل" onClick={()=>openEdit(r)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-gray-700 hover:bg-gray-50">
                            <Pencil className="h-4 w-4" />
                          </button>
                          <button title="أرشفة" onClick={()=>onDelete(r.id)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-rose-700 hover:bg-rose-50">
                            <Archive className="h-4 w-4" />
                          </button>
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-1">
                          <button title="استرجاع" onClick={()=>onRestore(r.id)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-emerald-700 hover:bg-emerald-50">
                            <RotateCcw className="h-4 w-4" />
                          </button>
                          <button title="حذف نهائي" onClick={()=>onForceDelete(r.id)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-rose-700 hover:bg-rose-50">
        ...
        <dialog open className="fixed inset-0 z-50 flex items-center justify-center  backdrop-blur-sm rounded-2xl">
          <form onSubmit={onSubmit} className="w-[min(92vw,720px)] max-w-2xl rounded-2xl border border-orange-200/70 bg-white p-6 shadow-2xl sm:p-7" dir="rtl">
            <div className="mb-5 flex items-start justify-between">
              <div>
                <h3 className="text-xl font-extrabold tracking-tight text-gray-900">{editing ? "تعديل جهة" : "إضافة جهة"}</h3>
                <p className="mt-1 text-xs text-gray-500">أدخل أسماء الجهة بدقة كما تريد ظهورها في الموقع</p>
              </div>
              <button type="button" onClick={()=>{ setEditing(null); setForm({ name_ar: "", name_fr: "" }); setModalOpen(false); }} className="rounded-lg border px-2 py-1 text-sm hover:bg-gray-50">إغلاق</button>
            </div>
            ...
            <div className="mt-6 flex items-center justify-end gap-2">
              <button type="button" onClick={()=>{ setEditing(null); setForm({ name_ar: "", name_fr: "" }); setModalOpen(false); }} className="rounded-lg border px-3 py-2 text-sm hover:bg-gray-50">إلغاء</button>
            ...
            <div className="mb-3 flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-600">
                  {confirm.type === 'archive' ? 'سيتم نقل الجهة إلى المؤرشفة ويمكن استرجاعها لاحقاً.' : 'هذا الإجراء نهائي ولا يمكن التراجع عنه.'}
                </p>
              </div>
