import { Pencil, Archive, RotateCcw, Trash2, Star } from "lucide-react";
import { useEffect, useMemo, useState } from "react";

import DashboardLayout from "./DashboardLayout";
import { createTestimonial, deleteTestimonial, forceDeleteTestimonial, getTestimonials, restoreTestimonial, updateTestimonial, type Testimonial } from "../../services/testimonials";
...
export default function TestimonialsPage() {
  ...
  const [form, setForm] = useState<{ user_id: number; rating: number; comment: string; is_approved: boolean }>({ user_id: 1, rating: 5, comment: "", is_approved: true });
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await getTestimonials(status);
        setRows(data);
      } catch {
  ...
  const filtered = useMemo(() => {
    ...
    if (!t) return rows;
    return rows.filter(r => r.comment.toLowerCase().includes(t) || String(r.rating).includes(t) || (r.user?.name || '').toLowerCase().includes(t));
  }, [rows, q]);

  const RatingStars: React.FC<{ value: number }> = ({ value }) => {
    return (
      <div className="inline-flex items-center" aria-label={`التقييم ${value} من 5`}>
        {Array.from({ length: 5 }).map((_, i) => (
          <Star key={i} className={`h-4 w-4 ${i < value ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`} />
        ))}
        <span className="ms-1 text-[11px] text-gray-500">{value}/5</span>
      </div>
    );
  };
  ...
  const toggleApprove = async (id: number, current: boolean) => {
    // optimistic update
    setRows(prev => prev.map(r => (r.id === id ? { ...r, is_approved: !current } : r)));
    try {
      await updateTestimonial(id, { is_approved: !current });
    } catch {
      // rollback
      setRows(prev => prev.map(r => (r.id === id ? { ...r, is_approved: current } : r)));
      alert('تعذر تغيير حالة الموافقة');
    }
  };

  const openCreate = () => {
    setEditing(null);
    setForm({ user_id: 1, rating: 5, comment: "", is_approved: true });
    setModalOpen(true);
  };
  const openEdit = (t: Testimonial) => {
    setEditing(t);
    setForm({ user_id: t.user_id, rating: t.rating, comment: t.comment, is_approved: !!t.is_approved });
    setModalOpen(true);
  };
  ...
  const confirmAction = async () => {
    ...
    try {
      if (confirm.type === 'archive') {
        await deleteTestimonial(confirm.id);
      } else {
        await forceDeleteTestimonial(confirm.id);
      }
  ...
  return (
    <DashboardLayout title="الشهادات (Testimonials)">
      <section className="rounded-2xl border border-orange-200/60 bg-white p-0 shadow-sm" dir="rtl">
        <div className="flex items-center justify-between gap-2 rounded-t-2xl border-b border-orange-200/50 bg-gradient-to-br from-orange-50 to-amber-50 px-4 py-3">
          <div>
            <h3 className="text-sm font-extrabold tracking-tight text-orange-900">Testimonials</h3>
            <p className="text-xs text-gray-600">إدارة الشهادات والتعليقات</p>
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="text-gray-600">
                <tr className="bg-gray-50">
                  ...
                  <th className="px-3 py-2 text-start font-semibold">التقييم</th>
                  <th className="px-3 py-2 text-start font-semibold">التعليق</th>
                  <th className="px-3 py-2 text-start font-semibold">الموافقة</th>
                  <th className="px-3 py-2 text-end font-semibold">إجراءات</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {filtered.map((t) => (
                  <tr key={t.id} className="transition hover:bg-orange-50/40">
                    <td className="px-3 py-3 text-gray-700">{t.user?.name ?? `#${t.user_id}`}</td>
                    <td className="px-3 py-3 text-gray-700"><RatingStars value={t.rating} /></td>
                    <td className="px-3 py-3 text-gray-700 max-w-[40ch] truncate" title={t.comment}>{t.comment}</td>
                    <td className="px-3 py-3 text-gray-700">
                      <button
                        type="button"
                        role="switch"
                        aria-checked={t.is_approved}
                        onClick={()=>toggleApprove(t.id, !!t.is_approved)}
                        className={`relative h-6 w-11 rounded-full transition ${t.is_approved ? 'bg-emerald-500' : 'bg-gray-300'}`}
                        title={t.is_approved ? 'إلغاء الموافقة' : 'تفعيل الموافقة'}
                      >
                        <span className={`absolute top-0.5 h-5 w-5 transform rounded-full bg-white shadow transition ${t.is_approved ? 'translate-x-5' : 'translate-x-1'}`}/>
                      </button>
                    </td>
                    <td className="px-3 py-3 text-end">
                      {status !== 'archived' ? (
                        <div className="inline-flex items-center gap-1">
                          <button title="تعديل" onClick={()=>openEdit(t)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-gray-700 hover:bg-gray-50">
                            <Pencil className="h-4 w-4" />
                          </button>
                          <button title="أرشفة" onClick={()=>onDelete(t.id)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-rose-700 hover:bg-rose-50">
                            <Archive className="h-4 w-4" />
                          </button>
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-1">
                          <button title="استرجاع" onClick={()=>onRestore(t.id)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-emerald-700 hover:bg-emerald-50">
                            <RotateCcw className="h-4 w-4" />
                          </button>
                          <button title="حذف نهائي" onClick={()=>onForceDelete(t.id)} className="inline-flex items-center justify-center rounded-md border bg-white p-1 text-rose-700 hover:bg-rose-50">
        ...
        <dialog open className="fixed inset-0 z-50 flex items-center justify-center  backdrop-blur-sm rounded-2xl">
          <form onSubmit={onSubmit} className="w-[min(92vw,720px)] max-w-2xl rounded-2xl border border-orange-200/70 bg-white p-6 shadow-2xl sm:p-7" dir="rtl">
            <div className="mb-5 flex items-start justify-between">
              <div>
                <h3 className="text-xl font-extrabold tracking-tight text-gray-900">{editing ? "تعديل شهادة" : "إضافة شهادة"}</h3>
                <p className="mt-1 text-xs text-gray-500">أدخل بيانات الشهادة</p>
              </div>
            ...
            <div className="grid gap-4 sm:gap-5">
              <label className="block text-sm">
                <span className="mb-1.5 block text-gray-700">معرف المستخدم (user_id)</span>
                <input required type="number" min={1} value={form.user_id} onChange={e=>setForm(v=>({ ...v, user_id: Number(e.target.value) }))} className="w-full rounded-lg border px-3 py-2.5 text-gray-900 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100" />
              </label>
              <div className="block text-sm">
                <span className="mb-1.5 block text-gray-700">التقييم</span>
                  ...
                  {Array.from({ length: 5 }).map((_, i) => {
                    ...
                    return (
                      <button
                        ...
                        aria-label={`اختر ${v}`}
                      >
                        <Star className={`h-6 w-6 ${v <= form.rating ? 'fill-amber-400 text-amber-400' : 'text-gray-300 hover:text-amber-400'}`} />
                      </button>
                    );
                  })}
                </div>
              ...
              <label className="block text-sm">
                <span className="mb-1.5 block text-gray-700">التعليق</span>
                <textarea required value={form.comment} onChange={e=>setForm(v=>({ ...v, comment: e.target.value }))} className="w-full rounded-lg border px-3 py-2.5 text-gray-900 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100" />
              </label>
              {/* Removed approval switch from form (approval can be toggled inline in the table) */}
            </div>
